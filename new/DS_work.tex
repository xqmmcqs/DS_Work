...
//打开窗口，控制窗口的显示和关闭
	...
	//释放链表空间
		-isEmpty(const List list)
		-delNode(List list, List pnode)
/par其中 $nextNode()$函数和$delNode()$函数是实现约瑟夫环的核心，通过不断向后找到节点并删除来实现逐个退圈的目标
...
\begin{enumerate}
	...
	\item -bool isEmpty(const List list)
	{	
		判断链表是否下一个节点是否指向自身，是则返回1，不是则返回0；
	}
	\item -void addNode(List pnode, int item) 
	{	
		给addNode分配内存
		if(addNode内存分配失败)
			异常退出
	...
	\item -List nextNode(const List pnode)
	{
		定义nItem为pnode的后继;
    	if (nItem是头节点) 
       		nItem 指向它的后继;
	...
	\item -void delNode(List list, List pnode) 
	{
		定义delNode为pnode的后继;
		if (delNode 是头节点List)
			pnode和 delNode都指向他们的后继;
		释放delNode;
	}
	\item -void destroyList(List * plist)
	{
		While(*plist不为空);
			删除*plist指向的节点;
		释放*plist;
...
\includegraphics[width=4]{pic1.png}
